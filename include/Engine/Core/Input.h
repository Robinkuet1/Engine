...
/**
 * Use this enum instead of GLFW constants
 */
enum Key {
  KEY_UNKNOWN = -1,
  KEY_SPACE = 32,
...
/**
 * This is a static class that should be used for Input devices.
 */
class Input {
public:
 /**
  * \brief Initializes Init system. Needs to be called once after window creation.
  */
 static void Initialize();
  ...
  static void OnKeyDown(std::function<void(Key)> f);
  static void OnKeyDown(Key k, std::function<void()> f);
  static void OnKeyUp(std::function<void(Key)> f);
  ...
  inline static std::unordered_map<Key, std::vector<std::function<void()>>>
      OnSpecificKeyDownCallback;
};
